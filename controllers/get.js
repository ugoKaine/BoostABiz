const Product = require("../models/store");
const Receipt = require("../models/receipt");
const User = require("../models/user");

exports.getStore = (req, res, next) => {
  const role = req.session.user.role;
  if (role !== "admin") {
    req.session.destroy();
    return res.redirect("/");
  }
  dataL = [];
  Product.find({})
    .sort("title")
    .then((product) => {
      if (product) {
        product.forEach((p) => {
          dataL.push(p.title);
        });
        dataL.sort();
        res.render("admin/store", {
          prods: product,
          prod: dataL,
          errorMessage: req.flash("error"),
          role: role,
        });
      }
    })
    .catch((err) => {
      if (err) {
        console.log(err);
      }
    });
};

exports.getReceipt = (req, res, next) => {
  const role = req.session.user.role;
  dataL = [];
  Product.find({})
    .sort("title")
    .then((product) => {
      if (product) {
        product.forEach((p) => {
          dataL.push({"title":p.title,"price":p.price});
        });

        dataL.sort(dataL.title)
        res.render("user/generateReceipt", { prod: dataL, role: role });
      }
    })
    .catch((err) => {
      console.log(err);
    });
};

exports.getSales = (req, res, next) => {
  const role = req.session.user.role;
  if (role !== "admin") {
    req.session.destroy();
    return res.redirect("/");
  }
  const userL = [];
  const superAdmin = [];
  TotalS = 0;
  User.find().then((user) => {
    if (user) {
      user.forEach((u) => {
        if (u.username != "destiny.ochonogor@gmail.com") {
          userL.push(u.username);
        } else {
          superAdmin.push(u.username);
        }
      });
    }
  });
  Receipt.find({
    createdAt: {
      $gte: new Date(new Date().setHours(23, 59, 59, 999)) - 8.64e7,
    },
  })
    .sort({ createdAt: -1 })
    .then((receipt) => {
      receipt.forEach((rep) => {
        TotalS += rep.grandTotal;
      });
      res.render("admin/checkUser", {
        receipt: receipt,
        totalSales: TotalS,
        role: role,
        u: userL,
      });
    })
    .catch((err) => {
      console.log(err);
    });
};

exports.getIndex = (req, res, next) => {
  const role = req.session.user.role;
  res.render("user/index", { role: role });
};

exports.getHome = (req, res, next) => {
  const role = req.session.user.role;
  res.render("user/index", { role: role });
};

exports.getUsers = (req, res, next) => {
  const role = req.session.user.role;
  User.find({ username: { $ne: "destiny.ochonogor@gmail.com" } })
    .then((user) => {
      res.render("admin/users", { role: role, user: user });
    })
    .catch((err) => {
      console.log(err);
    });
};
